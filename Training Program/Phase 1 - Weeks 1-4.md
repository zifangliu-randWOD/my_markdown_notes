# Phase 1: Positional Foundation (Weeks 1–8)

## Phase Goals

- Rebuild snatch and jerk positions through high-volume complexes at moderate loads
- Build squat volume base (sets of 5–8) to drive hypertrophy and tendon adaptation
- Begin left knee rehab — unilateral work every session
- Unlock thoracic mobility for front rack and jerk
- Establish gymnastics skill practice habits (ring MU, HS walk)

## Phase 1 Weekly Template

| Day | Primary | Secondary | Accessory (15-20 min) |
|---|---|---|---|
| Monday | Snatch complexes | Back squat (volume) | Knee rehab + thoracic mobility |
| Tuesday | Jerk from rack + Strict press | Ring MU progressions | Front rack mobility + shoulder prehab |
| Wednesday | Clean & deadlift | — | Unilateral leg work |
| Thursday | Front squat + OHS | Metcon (12-15 min) | Thoracic mobility + ankle work |
| Friday | Snatch (full lifts) + C&J | HS walk progressions | Knee rehab + hip mobility |

> [!tip] Phase 1 Coaching Cues
> - Every snatch rep: think "knuckles down, elbows high" in the pull, "punch up and press out" in the catch
> - Jerk dip: "vertical torso, heels down, elbows forward" — if knee pain appears, reduce load and add tempo
> - Film your lifts. Review between sets. One cue at a time.

---

## Week 1 — Introduction & Baseline

> [!info] Week 1 Targets
> Oly intensity: 70-75% | Squat intensity: 70% | Focus: establish positions, find working weights

### Monday — Snatch Complex + Back Squat

**Warm-up (10 min):** Thoracic foam roll, banded pull-aparts, PVC snatch grip behind-the-neck press, overhead squat with empty bar, snatch balance x5

**Primary: Snatch Complex**
- Complex: Hang snatch (above knee) + Snatch from floor + OHS
- 5 sets x 1 complex @ 70% (73 kg)
- Rest 2:00 between sets
- Focus: consistent pull path, patient off the floor

**Secondary: Back Squat**
- 4 x 6 @ 60% (115 kg)
- Tempo: 3 seconds down, 1 second pause at bottom, explode up
- Focus: even weight distribution left/right, find consistent bar position
- *Adjusted from 70% (133 kg) — tempo adds ~10-15% effective load; 133 kg was RPE 8-9 with failed reps on Day 1*

**Accessory Window (15-20 min):**
- Peterson step-downs: 3 x 10 each leg (bodyweight or light DB)
- Spanish squat iso hold: 3 x 30 sec
- Thoracic extension over foam roller: 3 x 10 breaths

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Snatch complex notes:
- Squat notes:
- Left knee feel (1-5, 5=no pain):
- General notes:

---

### Tuesday — Jerk + Strict Press + Ring MU Skill

**Warm-up (10 min):** Banded lat stretch, front rack stretch with barbell, thoracic rotation on wall, push press with empty bar x10, jerk footwork drills

**Primary: Jerk from Rack**
- Push jerk: 5 x 3 @ 70% (84 kg)
- Focus: vertical dip, elbows forward, fast lockout
- If knee pain in dip: slow the dip tempo to 2 sec, reduce load 5-10%

**Secondary: Strict Press**
- 4 x 6 @ moderate (start ~60 kg, adjust by feel)
- Focus: full lockout overhead, no excessive back lean

**Gymnastics Skill: Ring Muscle-Up Progressions (5-10 min)**
- Deep ring dips: 3 x 5 (slow negative)
- False grip hang: 3 x 15 sec
- Banded ring muscle-up transitions: 3 x 3

**Accessory Window (15-20 min):**
- Banded front rack stretch: 2 x 45 sec each side
- Prone Y-T-W raises: 3 x 8 each position
- External rotation with band: 3 x 12 each arm

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Jerk notes (dip feel, elbow position):
- Left arm front rack feel (1-5, 5=no restriction):
- Ring MU progression notes:
- General notes:

---

### Wednesday — Clean & Deadlift

**Warm-up (10 min):** Hip circles, banded good mornings, clean grip deadlift to knee x5, hang muscle clean x5, front squat with empty bar x5

**Primary: Clean Work**
- Power clean + Squat clean: 5 x (1+1) @ 70% (98 kg)
- Focus: smooth pull, elbows through fast in the catch

**Secondary: Deadlift**
- Clean-grip deadlift: 4 x 5 @ 75% of deadlift (150 kg)
- Tempo: 3 sec up, 3 sec down
- Focus: maintain back angle off the floor, even pressure through feet

**Accessory Window (15-20 min):**
- Bulgarian split squat: 3 x 8 each leg (DB or KB)
- Single-leg RDL: 3 x 8 each leg
- Banded hip flexor stretch: 2 x 45 sec each side

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Clean notes:
- Deadlift notes:
- Left vs right feel on unilateral work:
- General notes:

---

### Thursday — Front Squat + OHS + Metcon

**Warm-up (10 min):** Thoracic foam roll, front rack stretch, goblet squat x10, OHS with empty bar x5, snatch grip push press behind neck x5

**Primary: Front Squat**
- 4 x 6 @ 70% (112 kg)
- Tempo: 3 sec down, no pause, drive up
- Focus: elbows high throughout, upright torso

**Secondary: Overhead Squat**
- 4 x 5 @ 60% of snatch (63 kg)
- Focus: active shoulders, midline stability, full depth

**Metcon (12-15 min):**
- 3 rounds for time:
  - 12 thrusters (43 kg)
  - 12 pull-ups
  - 200m run

**Accessory Window (15-20 min):**
- Thoracic extension over bench with barbell: 3 x 10 breaths
- Ankle dorsiflexion stretch (wall): 2 x 30 sec each
- Banded overhead distraction: 2 x 30 sec each arm

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Front squat notes (left knee feel):
- OHS notes (overhead stability):
- Metcon time:
- General notes:

---

### Friday — Snatch + C&J + HS Walk Skill

**Warm-up (10 min):** Snatch grip RDL x5, hang snatch high pull x5, snatch balance x5, clean & jerk with empty bar x3

**Primary: Snatch (Full Lifts)**
- Work to a comfortable heavy single @ 75% (79 kg), then:
- 5 x 2 @ 70-75% (73-79 kg)
- Focus: speed under the bar, consistent catch position

**Secondary: Clean & Jerk**
- 4 x (1+1) @ 70% (clean 98 kg / jerk 84 kg — use 93 kg as working weight)
- Focus: smooth transition from clean to jerk, reset feet between

**Gymnastics Skill: Handstand Walk Progressions (5-10 min)**
- Wall-facing handstand hold: 3 x 30 sec
- Wall walk with shoulder taps: 3 x 5 each hand
- Freestanding handstand kick-up attempts: 5 attempts

**Accessory Window (15-20 min):**
- Peterson step-downs: 3 x 10 each leg
- Pigeon stretch: 2 x 45 sec each side
- 90/90 hip rotation: 2 x 8 each side

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Snatch notes (positions, speed):
- C&J notes:
- HS walk progress:
- Left knee end-of-week feel (1-5):
- General notes:

---

> [!note] Week 1 Reflection
> **Overall feel:** Good baseline week. Oly lifts on target; strength work was miscalibrated high but self-corrected within each session. 5/5 sessions completed.
>
> **What went well:** Snatch drive position migrating higher (complex work paying off). Push jerk right on target at RPE 6. Clean felt strong all week. No missed Oly lifts.
>
> **What needs attention:** Tempo squat calibration (resolved — all reduced to 60-65%). Jerk dip confidence. Jerk grip development (narrow grip identified as limiter). Wednesday unilateral work missed (off-program box session).
>
> **Left knee status:** Stable 4-5/5. Jerk dip is the specific provocative position. No lingering post-session pain. Rehab exercises completed 2x/week as prescribed.
>
> **Front rack / thoracic progress:** Baseline established. Not limiting at 70-75% loads. Shows up under fatigue (front squat bar slide on later reps) and in jerk grip limitation.
>
> **Adjustments for next week:** Tempo squat loads reduced to 60-62%. Strict press recalibrated to 52 kg. Snatch squat-receive cue added. Jerk grip exploration during warm-ups.
>
> See [[Week 1 - Summary & Diagnosis]] for full analysis.

---

## Week 2 — Building Volume

> [!info] Week 2 Targets
> Oly intensity: 72-77% | Squat intensity: 60-62% | Focus: add 1 set or 2-3% to Oly lifts, keep tempo squats at calibrated loads
>
> **Week 1 adjustments applied:**
> - Tempo squat loads reduced (70% was RPE 8-9; now targeting 60-62% for RPE 6-7)
> - Strict press working weight recalibrated to 52 kg (from 60 kg base)
> - Snatch working sets: cue squat-receiving even when power snatching is possible
> - Jerk warm-ups: experiment with grip 1 inch wider than clean grip on light sets

### Monday — Snatch Complex + Back Squat

**Warm-up:** Same as Week 1

**Primary: Snatch Complex**
- Complex: Pause snatch (2 sec at knee) + Snatch from floor + OHS
- 6 sets x 1 complex @ 72% (76 kg)
- Rest 2:00
- **New cue:** On the snatch from floor, intentionally ride the bar down into a full overhead squat catch even if you could power it. Build the receiving position habit.

**Secondary: Back Squat**
- 4 x 6 @ 60% (115 kg)
- Tempo: 3-1-0
- *Week 1 at 60% (115 kg) after dropping from 133 kg was still RPE 8-9. If 115 kg still feels like RPE 8+, drop to 110 kg. The goal is RPE 6-7 with perfect positions.*

**Accessory Window:**
- Peterson step-downs: 3 x 10 each leg (add light DB — Week 1 was bodyweight)
- Spanish squat iso hold: 3 x 35 sec
- Thoracic extension over foam roller: 3 x 10 breaths

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Snatch complex notes:
- Squat notes:
- Left knee feel (1-5):
- General notes:

---

### Tuesday — Jerk + Strict Press + Ring MU Skill

**Warm-up:** Same as Week 1
- **Added:** During push press warm-up, try 2-3 reps with grip 1 inch wider than clean grip. Note how the dip and overhead position feel at the wider width. This is exploratory — not a commitment to switch.

**Primary: Jerk from Rack**
- Push jerk: 5 x 3 @ 72% (86 kg)
- **Dip cue from Week 1:** The dip is slow/tentative due to knee apprehension. Don't force speed — focus on a vertical torso and consistent depth. Speed comes from confidence, and confidence comes from pain-free repetitions. As the dip feels safer, tempo will naturally increase.

**Secondary: Strict Press**
- 4 x 6 @ 52 kg
- *Recalibrated: Week 1 showed 50 kg is the working weight for sets of 6 (60 kg was RPE 8-9). Building +2 kg from 50 kg.*

**Gymnastics Skill: Ring MU Progressions**
- Deep ring dips: 3 x 6
- False grip hang: 3 x 20 sec (Week 1 identified grip as the limiter — this is the priority)
- Banded ring MU transitions: 3 x 3 (lighter band if possible)

**Accessory Window:**
- Banded front rack stretch: 2 x 45 sec each
- Prone Y-T-W raises: 3 x 10
- External rotation: 3 x 12 each

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Jerk notes (dip feel, grip experiment):
- Ring MU progress:
- General notes:

---

### Wednesday — Clean & Deadlift

**Warm-up:** Same as Week 1

**Primary: Clean Work**
- Power clean + Squat clean: 5 x (1+1) @ 72% (101 kg)

**Secondary: Deadlift**
- Clean-grip deadlift: 4 x 5 @ 77% (154 kg)
- Tempo: 3-0-3
- *Monitor RPE: if tempo deadlifts follow the same pattern as tempo squats (RPE 8+ at prescribed %), drop to 140-145 kg.*

**Accessory Window:**
- Bulgarian split squat: 3 x 10 each leg (**do not skip** — Wednesday unilateral work was missed in Week 1)
- Single-leg RDL: 3 x 10 each leg
- Banded hip flexor stretch: 2 x 45 sec each

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Clean notes:
- Deadlift notes:
- Left vs right feel on unilateral work:
- General notes:

---

### Thursday — Front Squat + OHS + Metcon

**Warm-up:** Same as Week 1

**Primary: Front Squat**
- 4 x 6 @ 65% (104 kg)
- Tempo: 3-0-0
- *Adjusted from 72% (115 kg) — Week 1 at 70% (112 kg) was RPE 8+ with failed reps; tempo adds ~10-15% effective load. Athlete settled at 100-105 kg as working range. Target RPE 6-7.*
- **Cue from Week 1:** "Elbows UP" before every rep, not just at the start of the set. Bar slides forward when thoracic extensors fatigue — this is the cue to fight it.

**Secondary: Overhead Squat**
- 4 x 5 @ 63% of snatch (66 kg)
- **Grip cue from Week 1:** Soft hands, hard shoulders — don't death-grip the bar. Widen grip by 1-2 finger widths if wrist pressure persists. Bar should rest on the heel of the palm, not across the middle. Shakiness during descent is normal and will improve with practice.

**Metcon (12-15 min):**
- AMRAP 12 min:
  - 10 KB swings (32 kg)
  - 8 box jumps (24")
  - 6 burpees

**Accessory Window:**
- Thoracic extension over bench: 3 x 10 breaths
- Ankle dorsiflexion: 2 x 30 sec each
- Banded overhead distraction: 2 x 30 sec each

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Front squat notes:
- OHS notes:
- Metcon score:
- General notes:

---

### Friday — Snatch + C&J + HS Walk Skill

**Warm-up:** Same as Week 1

**Primary: Snatch**
- Work to comfortable heavy single @ 77% (81 kg), then:
- 5 x 2 @ 72-75% (76-79 kg)
- **Cue from Week 1:** Intentionally ride the bar down into a squat receive on working sets, even if you can power snatch it. Building this habit now prepares for heavier loads later.

**Secondary: Clean & Jerk**
- 4 x (1+1) @ 72% (95 kg working weight)
- **Jerk grip:** Try moving hands 1 inch wider than clean grip on sets 3-4. If it feels unstable, return to clean grip. This is exploration, not commitment.

**Gymnastics Skill: HS Walk Progressions**
- Wall-facing handstand hold: 3 x 35 sec
- Wall walk with shoulder taps: 3 x 6 each hand
- Freestanding HS kick-up: 5 attempts (try to hold 3 sec)

**Accessory Window:**
- Peterson step-downs: 3 x 10 each (add light DB — Week 1 was bodyweight, time to progress)
- Pigeon stretch: 2 x 45 sec each
- 90/90 hip rotation: 2 x 8 each

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Snatch notes (did you squat-receive?):
- C&J notes (grip experiment?):
- HS walk progress:
- Left knee end-of-week feel (1-5):
- General notes:

---

> [!note] Week 2 Reflection
> **Overall feel:**
>
> **What went well:**
>
> **What needs attention:**
>
> **Left knee status:**
>
> **Front rack / thoracic progress:**
>
> **Adjustments for next week:**

---

## Week 3 — Peak Volume

> [!info] Week 3 Targets
> Oly intensity: 75-80% | Squat intensity: 75% | Focus: highest volume week before deload, push sets/reps

### Monday — Snatch Complex + Back Squat

**Warm-up:** Same protocol

**Primary: Snatch Complex**
- Complex: Hang snatch (below knee) + Pause snatch (at knee) + Snatch
- 6 sets x 1 complex @ 75% (79 kg)
- Rest 2:00-2:30

**Secondary: Back Squat**
- 5 x 5 @ 65% (124 kg)
- Tempo: 3-1-0

**Accessory Window:**
- Peterson step-downs: 3 x 12 each leg (with DB)
- Spanish squat iso hold: 3 x 40 sec
- Thoracic extension: 3 x 10 breaths

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Snatch complex notes:
- Squat notes:
- Left knee feel (1-5):
- General notes:

---

### Tuesday — Jerk + Push Press + Ring MU Skill

**Warm-up:** Same protocol

**Primary: Jerk from Rack**
- Push jerk: 6 x 3 @ 75% (90 kg)

**Secondary: Push Press** (replacing strict press this week for overload)
- 4 x 5 @ moderate-heavy (start ~70 kg)

**Gymnastics Skill: Ring MU Progressions**
- Deep ring dips: 3 x 8
- False grip pull-up: 3 x 3 (new progression)
- Banded ring MU transitions: 4 x 3

**Accessory Window:**
- Banded front rack stretch: 2 x 60 sec each
- Prone Y-T-W raises: 3 x 10
- External rotation: 3 x 15 each

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Jerk notes:
- Push press notes:
- Ring MU progress:
- General notes:

---

### Wednesday — Clean & Deadlift

**Warm-up:** Same protocol

**Primary: Clean Work**
- Hang clean (below knee) + Squat clean: 6 x (1+1) @ 75% (105 kg)

**Secondary: Deadlift**
- Clean-grip deadlift: 5 x 5 @ 78% (156 kg)
- Tempo: 3-0-3

**Accessory Window:**
- Bulgarian split squat: 3 x 10 each leg (heavier DB)
- Single-leg RDL: 3 x 10 each leg
- Hip flexor stretch: 2 x 60 sec each

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Clean notes:
- Deadlift notes:
- General notes:

---

### Thursday — Front Squat + OHS + Metcon

**Warm-up:** Same protocol

**Primary: Front Squat**
- 5 x 5 @ 67% (107 kg)
- Tempo: 3-0-0
- *Adjusted from 75% (120 kg) — consistent pattern: tempo front squats at 70%+ exceed target RPE. Building from Week 2's 104 kg.*

**Secondary: Overhead Squat**
- 4 x 5 @ 65% of snatch (68 kg)

**Metcon (15 min):**
- For time:
  - 21-15-9
  - Wall balls (9 kg)
  - Toes-to-bar

**Accessory Window:**
- Thoracic extension over bench: 3 x 10 breaths
- Ankle dorsiflexion: 2 x 30 sec each
- Overhead distraction: 2 x 30 sec each

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Front squat notes:
- OHS notes:
- Metcon time:
- General notes:

---

### Friday — Snatch + C&J + HS Walk Skill

**Warm-up:** Same protocol

**Primary: Snatch**
- Work to heavy single @ 80% (84 kg), then:
- 5 x 2 @ 75% (79 kg)

**Secondary: Clean & Jerk**
- 5 x (1+1) @ 75% (98 kg working weight)

**Gymnastics Skill: HS Walk Progressions**
- Wall-facing handstand hold: 3 x 40 sec
- Wall walk with shoulder taps: 3 x 8 each hand
- Freestanding HS: attempt 3-5 sec holds, 5 attempts
- Wall-facing HS lateral weight shifts: 3 x 5 each direction (new)

**Accessory Window:**
- Peterson step-downs: 3 x 12 each (with DB)
- Pigeon stretch: 2 x 60 sec each
- 90/90 hip rotation: 2 x 10 each

**Tracking:**
- [ ] Completed
- RPE: _ /10
- Snatch notes:
- C&J notes:
- HS walk progress:
- Left knee end-of-week feel (1-5):
- General notes:

---

> [!note] Week 3 Reflection
> **Overall feel:**
>
> **What went well:**
>
> **What needs attention:**
>
> **Left knee status:**
>
> **Front rack / thoracic progress:**
>
> **Adjustments for next week:**

---

## Week 4 — Deload

> [!info] Week 4 Targets
> Oly intensity: 70-75% | Squat intensity: 60-70% | Volume cut ~40% | Focus: recover, maintain positions, increase mobility work

### Monday — Light Snatch + Easy Squat

**Warm-up:** Extended — 15 min mobility focus (thoracic, hips, ankles)

**Primary: Snatch**
- Snatch from floor: 3 x 2 @ 70% (73 kg) — focus on speed and positions only

**Secondary: Back Squat**
- 3 x 5 @ 55% (105 kg) — easy, no grinders

**Accessory Window:**
- Peterson step-downs: 3 x 10 each (maintain, don't increase)
- Extended thoracic mobility: 10 min dedicated work
- Spanish squat: 3 x 30 sec

**Tracking:**
- [ ] Completed
- RPE: _ /10 (should be ≤6)
- Notes:

---

### Tuesday — Light Jerk + Press

**Warm-up:** Extended mobility — front rack focus

**Primary: Jerk from Rack**
- Push jerk: 3 x 3 @ 70% (84 kg) — fast and crisp

**Secondary: Strict Press**
- 3 x 8 @ light (~50 kg)

**Gymnastics Skill: Ring MU**
- Maintain current progression: 2 sets only, quality reps

**Accessory Window:**
- Extended front rack and shoulder mobility: 15 min

**Tracking:**
- [ ] Completed
- RPE: _ /10 (should be ≤6)
- Notes:

---

### Wednesday — Light Clean

**Warm-up:** Extended hip and hamstring mobility

**Primary: Clean**
- Squat clean: 3 x 2 @ 70% (98 kg)

**Secondary: Deadlift**
- Clean-grip deadlift: 3 x 3 @ 70% (140 kg) — easy

**Accessory Window:**
- Bulgarian split squat: 2 x 8 each (light)
- Single-leg RDL: 2 x 8 each (light)
- Extended hip mobility: 10 min

**Tracking:**
- [ ] Completed
- RPE: _ /10 (should be ≤6)
- Notes:

---

### Thursday — Light Front Squat + Fun Metcon

**Warm-up:** Extended thoracic and ankle mobility

**Primary: Front Squat**
- 3 x 5 @ 65% (104 kg)

**Secondary: OHS**
- 3 x 5 @ 55% of snatch (58 kg)

**Metcon (optional, keep it fun):**
- 10 min AMRAP, easy pace:
  - 10 cal row
  - 10 air squats
  - 10 push-ups

**Accessory Window:**
- Full mobility circuit: thoracic, ankles, hips — 15 min

**Tracking:**
- [ ] Completed
- RPE: _ /10 (should be ≤5)
- Notes:

---

### Friday — Light Snatch + C&J

**Warm-up:** Full mobility warm-up

**Primary: Snatch**
- 3 x 2 @ 70% (73 kg) — speed and positions

**Secondary: Clean & Jerk**
- 3 x (1+1) @ 70% (90 kg working weight)

**Gymnastics Skill: HS Walk**
- Maintain current progression: 2 sets only

**Accessory Window:**
- Peterson step-downs: 2 x 10 each (maintain)
- Full hip mobility circuit: 10 min

**Tracking:**
- [ ] Completed
- RPE: _ /10 (should be ≤5)
- Left knee end-of-week feel (1-5):
- Notes:

---

> [!note] Week 4 (Deload) Reflection
> **Recovery status (1-10, 10=fully recovered):**
>
> **Aches/pains that lingered vs resolved:**
>
> **Left knee compared to Week 1:**
>
> **Front rack compared to Week 1:**
>
> **Motivation level heading into Week 5:**
>
> **Anything to adjust for Block 2 (Weeks 5-8):**