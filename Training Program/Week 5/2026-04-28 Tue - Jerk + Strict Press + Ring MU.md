---
date: 2026-04-28
week: 5
phase: 1
day: Tuesday
training: Jerk + Strict Press + Ring MU
tags:
  - training
  - phase-1
  - week-5
---

# Tuesday — Week 5: Jerk + Strict Press + Ring MU Skill

[[00 - Program Overview]] | [[Phase 1 - Weeks 5-8]]

> [!tip] Day Focus
> **This is the heaviest test of the wider jerk stance since you adopted it.** Wk 3 Tuesday at 90 kg is the historical anchor — forward COM drift dropped the left knee to 3.5/5 mid-session and the wider stance (+5 cm outside hip-width) was deployed for the first time. It has since held knee 5/5 across every dedicated jerk session: Wk 4 Tue at 84 kg, Wk 4 Fri at 90 kg from the squat clean, and today's 92 kg is the first real load test of the embedded fix — +2 kg above Wk 3's working weight and +8 kg above last week's deload. Set up the wider stance from rep 1 of the empty-bar warm-up; **do not drift narrow under fatigue or inattention**, because the Wk 3 baseline is the failure mode this stance prevents. The bar-drift-toward-fingers pattern from Wk 4 Tuesday needs pre-emptive attention — seat the bar in the palm before each set, not at the finger base where it costs lockout stability under load. The heavy clean stand-up breathing protocol applies to the jerk dip rise too: controlled exhale through pursed lips on the drive, never Valsalva-and-release at the top. Strict press shifts strength-biased today — 65 kg × 5 instead of last week's 50 kg × 8 — because Wk 4's set 2 hit RPE 7.5 and identified endurance, not max strength, as the press limiter. Jerk RPE target is 7, press RPE target is 7; if any single set climbs to 7.5+, reset stance and breathing and continue at the same load, but if two consecutive sets land at 8+, drop the jerk to 88 kg or the press to 60 kg and finish. Ring MU progressions return to peak volume after the deload — 3 sets each, false grip pull-ups bump to 3–5 reps, banded transitions try the next-lighter band. **Success today is knee 5/5 across all five jerk sets, the wider stance feeling automatic by set 2, and the ring MU progression holding the volume bump** — strength is not being tested, durability of the embedded patterns is.

---

## Pre-Session (60–90 min before training)

- [ ] 500 ml water
- [ ] ~30 g carbs

---

## Warm-up (12 min)

- [ ] 90/90 breathing: 2 min
- [ ] **Right lumbar erector foam roll: 60 sec** (NEW)
- [ ] **Couch stretch: 60 sec each side** (NEW)
- [ ] Banded lat stretch: 45 sec each side
- [ ] Front rack stretch with barbell: 60 sec
- [ ] Thoracic rotation on wall: 8 each side
- [ ] Push press with empty bar: 10 reps
- [ ] Jerk footwork drills (split or power — whichever you use): 10 each side

> [!info]- Warm-up Movement Guide (click to expand)
>
> **Front rack stretch with barbell:** weekly check on left arm thoracic restriction. Compare today's elbow height to Wk 3's baseline. Has it improved? The Wk 4 mobility circuit (especially thoracic extension over bench) was specifically targeting this; the carryover should be visible.
>
> **Jerk footwork drills:** the wider stance is your default — set up the +5 cm wider stance from the very first drill. Don't drift to the old narrow stance under fatigue or absent attention. Build muscle memory at the right stance every rep.

---

## Primary: Jerk from Rack

**Push Jerk: 5 × 3 @ 77% (92 kg)**
- **Wider stance default (+5 cm outside hip-width)** — no exceptions
- Rest 2:00–2:30 between sets
- Focus: vertical dip, "heels heavy through the dip," elbows forward, fast lockout
- **Heavy clean stand-up breathing protocol applies** — controlled exhale through pursed lips during the rise from the dip; never Valsalva-and-release at the top

| Set | Weight | Reps | RPE | Knee (1-5) | Notes |
| --- | ------ | ---- | --- | ---------- | ----- |
| 1   | 92 kg  | /3   |     |            |       |
| 2   | 92 kg  | /3   |     |            |       |
| 3   | 92 kg  | /3   |     |            |       |
| 4   | 92 kg  | /3   |     |            |       |
| 5   | 92 kg  | /3   |     |            |       |

> [!info]- Jerk — Mechanism & Coaching Notes (click to expand)
>
> **Where 92 kg sits:** Your jerk PR is ~120 kg. 92 kg is 77%. Wk 3's 90 kg × 6×3 push jerk was RPE 6.5–7.5. Today is +2 kg with one more rep set — RPE target 7. The wider stance, now embedded, should keep the COM management clean and the knee response 5/5.
>
> **Wider stance default:**
> - +5 cm outside hip-width — same as Wk 3 Tuesday and Wk 4 Tuesday
> - Wider stance naturally shifts COM toward the heels during the dip — protects the patellar tracking on the left knee
> - Bar drift toward the fingers (cosmetic Wk 4 pattern) — pre-grip check before each set, seat the bar in the palm not the finger base
>
> **Heavy clean stand-up breathing — applied to jerk dip:**
> - In the dip: brace, hold air
> - As you initiate the drive: controlled exhale through pursed lips
> - At lockout: finish exhale, take 1 quick breath before re-racking
> - This reduces the post-Valsalva orthostatic risk that showed up Wk 4 Fri after the heavy clean
>
> **If knee discomfort appears at any rep:** stop the set, check the stance width (it may have drifted narrower), check the dip depth (too deep = COM forward). Do not push through. Drop load 5–10% if needed and continue.
>
> **RPE target:** 7. If a set climbs above 7.5, reset stance + breathing and continue at the same load. If two consecutive sets at 8+, reduce to 88 kg and finish remaining sets there.

---

## Secondary: Strict Press

**4 × 5 @ moderate-heavy (~65 kg, adjust by feel)**
- Standing, full lockout overhead, no excessive back lean
- Rest 2:00 between sets
- Focus: tight glutes + tight quads to prevent hyperextension; full reach overhead

| Set | Weight | Reps | RPE | Notes |
| --- | ------ | ---- | --- | ----- |
| 1   | 65 kg  | /5   |     |       |
| 2   | 65 kg  | /5   |     |       |
| 3   | 65 kg  | /5   |     |       |
| 4   | 65 kg  | /5   |     |       |

> [!info]- Strict Press — Coaching Notes (click to expand)
>
> **Wk 4's data:** 50 kg for 8 reps came in at RPE 7.5 on set 2 (deload load). That tells you shoulder pressing endurance is a real limiter; max strength is well above 50 kg but stamina across reps fatigues fast. Today is heavier (65 kg) and lower reps (5 instead of 8) — more strength-biased, less endurance-biased. RPE target 7 across all sets.
>
> **Bar position:** in the palm, not the fingers. Wrists stacked under the bar. If the bar drifts to the fingers, it's a forearm/grip strain risk and the lockout becomes less efficient.
>
> **Bar path:** straight up the face. Move the head out of the way as the bar passes the chin, then bar finishes overhead with shoulders shrugged into the ears. Do not press the bar forward and around.
>
> **If 65 kg feels harder than RPE 7 by set 2:** drop to 60 kg for sets 3–4 and note it. Press strength is a Phase 2 buildable; don't grind the first session.

---

## Gymnastics Skill: Ring Muscle-Up Progressions (5–10 min)

**3 sets each — quality over quantity. Stop each set 1–2 reps short of failure.**

- [ ] Deep ring dips: 3 × 8 (3-sec eccentric, full depth — rings outside armpits at bottom)
- [ ] False grip pull-ups: 3 × 3–5 (unassisted if possible; band assist if needed; maintain false grip throughout)
- [ ] Banded ring MU transitions: 4 × 2–3 (reduce band assistance from Wk 3 if possible)

> [!info]- Ring MU — Coaching Notes (click to expand)
>
> **Volume bump from Wk 4:** 3 sets each (vs 2 in deload). False grip pull-up rep range opens from 2–3 to 3–5 — if the false grip held cleanly during the deload, this is the week to push the rep count.
>
> **Banded MU transitions — band reduction:**
> - Goal of Wk 5–8 progression: lighter band each week if the transition holds. Today, try the next-lighter band you have. If the transition stalls or you can't get to support, drop back to the previous band for sets 2–4.
> - Cue: pull the rings to the hips, not the rings to the chest. The transition is a hip drive into a press-out, not a continuous pull.
>
> **False grip stamina:** if the grip slips by rep 3, that's the limiter, not pulling strength. Use a band on rep 4–5 to keep the false grip activation pattern intact rather than dropping the false grip.

---

## Accessory Window (15–20 min)

- [ ] Banded front rack stretch: 2 × 60 sec each side (extra time on left)
- [ ] Prone Y-T-W raises: 3 × 10 each position (1–2 kg each hand)
- [ ] External rotation with band: 3 × 15 each arm
- [ ] **Pigeon stretch: 60 sec each side** (left hip ER maintenance)

> [!info]- Accessory — Coaching Notes (click to expand)
>
> **Banded front rack stretch:** always extra time on the left. The thoracic + shoulder restriction is bilateral, worse on the left. 60 sec right, 70–75 sec left. Compare today's elbow drive in the front rack vs Wk 3's baseline — has it improved?
>
> **Prone Y-T-W raises:** lower trap + rhomboid activation. Without these the jerk lockout leans on the deltoids and biceps and the bar drifts forward at heavier loads. Slow, controlled, no momentum.
>
> **External rotation with band:** rotator cuff posterior chain. Two sets at 15 reps is maintenance; today bumps to 3 sets at 15 reps because Wk 5+ the OHS and snatch start loading the cuff more.

---

## Pre-Bed (5 min, training days only)

- [ ] Cat-cow: 10 reps slow
- [ ] Child's pose: 60 sec
- [ ] Supine spinal rotation: 5 each side, slow

---

## Daily Log

- [ ] Session completed
- RPE (overall): _ /10 (target 7)
- Left knee feel (1-5, 5=no pain): _
- Right lumbar feel today (1-5): _
- Jerk notes (wider stance natural? Knee response across 5 sets? Stand-up breathing applied?):
- Press notes (where did it become hard?):
- Ring MU notes (band reduction worked? False grip held?):
- Front rack feel (1-5, 5=no restriction): _
- Pre-session fueling done? Y/N: _
- Pre-bed mobility done? Y/N: _
- General notes:
